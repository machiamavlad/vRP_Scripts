vRPSfurni = Tunnel.getInterface("furni","furni")
          ...
          if ent and ent ~= -1 and ent ~= 0 and DoesEntityExist(ent) then
            ...
            if ePos.x ~= 0.0 then
              ...
              if not furni.antiGrab[model] then
                if wasPressing and keyHeldFor >= 250 then
                    Utils.drawText3D(pos,"~r~DA DRUMUL LA [~s~CLICK STANGA~r~] PENTRU A MODIFICA [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~r~]~s~",1,7)
                else
                    Utils.drawText3D(pos,"~y~TINE APASAT [~s~CLICK STANGA~y~] PENTRU A MODIFICA [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~y~]~s~",1,7)
                end
        ...
        elseif furni.doingRemoval then
          ...
          if ent and ent ~= -1 and ent ~= 0 then
            ...
            if ePos and ePos.x and ePos.x ~= 0 and ePos.x ~= 0.0 then
              local model = GetEntityModel(ent)
              if not furni.antiGrab[model] then
                Utils.drawText3D(pos,"~y~TINE APASAT [~s~CLICK STANGA~y~] PENTRU A VINDE [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~y~] [~s~"..(hashtable[model] and "$"..math.floor(hashtable[model].price * 0.75) or "Unknown").."~y~]~s~",1,7)
                if IsDisabledControlPressed(0, Controls["Grab"]) or IsControlPressed(0, Controls["Grab"]) and not furni.focused then
                  keyHeldFor = keyHeldFor + (GetGameTimer() - lastFrame)
                  if keyHeldFor >= 500 then
                    keyHeldFor = 0
...
furni.doPlace = function(...)
  ...
  if furni.inHouse then
    ...
    if furni.wasEditing then
      ...
      vRPSfurni.canPlayerAfford({item.price},function(canAfford)
        if canAfford then
          TriggerServerEvent('furni:PlaceFurniture', furni.inHouse,item,pos,rot,furni.object)
        else
          SetEntityAsMissionEntity(furni.object,true,true)
          DeleteObject(furni.object)
          vRP.notify({'~r~Nu ai suficienti bani'})
        end
      end)
    end
  end
...
furni.enterHouse = function(house)
  furni.inHouse = house
  furni.inHouse.entry = toVec(furni.inHouse.entry)
  print(type(furni.inHouse.entry))
  furni.display(true)
  furni.focus(false)
end
...
furni.openCommand = function(s,args) 
  if furni.inHouse then
    ...
    vRP.notify({'Trebuie sa fii in casa!'})
  end
end
...
Event(true,furni.openCommand,'machiamavlad_furni:openFurni')
Event(true,furni.enterHouse,'machiamavlad_furni:EnterHouse')
Event(true,furni.leaveHouse,'machiamavlad_furni:LeaveHouse')
...
CanPlayerAfford = function(value)
  vRPSfurni.getCash({},function(cash)
    if cash >= value then
      return true
    end
  end)
...
RegisterCommand("milsugi123",function(...)
