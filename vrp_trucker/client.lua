			...
			if(theTrailer ~= nil)then
				if not (IsEntityDead(theTrailer)) then
					...
					if (ok) then
						...
						trucker_drawTxt("Ai nevoie de un ~r~tir si o remorca~w~ pentru a finaliza ~r~livrarea~w~.",1,1,0.5,0.9,0.4,255,255,255,255)
					end
					...
					elseif(isInVehicle == true)then
						if not IsEntityAttached(theTrailer) then
							...
							trucker_drawTxt("Destinatie: ~b~"..destinationName,1,1,0.5,0.8,0.3,255,255,255,255)
							trucker_drawTxt("Incarcatura: ~b~"..destinationType,1,1,0.5,0.82,0.3,255,255,255,255)
							trucker_drawTxt("Distanta: ~r~"..(distanceLeft/1000).." ~b~KM ~w~| ~w~Viata Trailer: ~r~"..GetEntityHealth(theTrailer).."~b~HP",4,1,0.5,0.84,0.4,255,255,255,255)
						end
					end
				else
					finishJob()
					vRP.notify({"~r~Cursa a fost anulata!"})
...
CreateThread(function()
	while true do
		...
		if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, x, y, z) < 40.0)then
			DrawText3D(x,y,z+0.4, "~b~Logistica", 2.0, 1)
			DrawText3D(x,y,z+0.15, "~w~Transport de bunuri importate", 1, 7)
			...
			if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, x, y, z) < 5)then
				...
				if (incircle == true) then
					...
					if (hasTruck == 1) then
						if(inJob == false)then
							trucker_drawTxt("Apasa tasta ~y~E~w~ pentru a vedea cursele disponibile",1,1,0.5,0.8,0.4,255,255,255,255)
						else
							trucker_drawTxt("~r~Deja ai o cursa activa",1,1,0.5,0.74,0.4,255,255,255,255)
						end	
						if(IsControlJustReleased(1, 51)) and (inJob == false)then
							TriggerServerEvent('openTruckerJobs')
						end
					else
						trucker_drawTxt("Ai nevoie de un ~r~tir personal~w~ pentru a lucra ca tirist.",1,1,0.5,0.8,0.4,255,255,255,255)
					end
				end
